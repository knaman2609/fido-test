...
import { blocksToHTML, extractPlainText } from './blocknoteService.js';
...
class UIRendererImpl implements IUIRenderer {
  ...
  render(todos: Todo[]): void {
    ...
    todos.forEach(todo => {
      ...
      const plainText = extractPlainText(todo.content);
      checkbox.setAttribute('aria-label', `Mark "${plainText}" as ${todo.completed ? 'incomplete' : 'complete'}`);
      checkbox.setAttribute('data-id', todo.id.toString());
      checkbox.setAttribute('data-action', 'toggle');

      const contentDiv = document.createElement('div');
      contentDiv.className = 'todo-content';
      contentDiv.innerHTML = blocksToHTML(todo.content);

      const deleteBtn = document.createElement('button');
      deleteBtn.className = 'delete-btn';
      deleteBtn.textContent = 'Delete';
      deleteBtn.setAttribute('aria-label', `Delete "${plainText}"`);
